...
/// Calculates the attenuation value of a point light based on the given distance. The distance of the fragment from the 
/// light is represented by dist.
/// Returns an attenuation factor ranging from 0 to 1.
float attenuate (in const float dist);
...
float attenuate (in const float dist)
{
    // We need to construct 1 / (Kc + Kl * d + Kq * d * d).
    return 1.0 / (aConstant + aLinear * dist + aQuadratic * dist * dist);
}
